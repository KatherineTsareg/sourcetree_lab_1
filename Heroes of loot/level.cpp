...
void Level::DrawDynamicObjects(std::vector<Object> &obj, sf::RenderWindow &window, std::string name, sf::Sprite &sprite, int left, int top, float time, float &current_frame)
{
	for (int i = 0; i < obj.size(); i++)
		if (obj[i].name == name)
		{
			...
			{
				if (obj[i].name == "torch")
					sprite.setTextureRect(IntRect(left + obj[i].rect.width * (int)current_frame, top, obj[i].rect.width, obj[i].rect.height - 50));
				else
					sprite.setTextureRect(IntRect(left + obj[i].rect.width * (int)current_frame, top, obj[i].rect.width, obj[i].rect.height));
				sprite.setPosition(obj[i].rect.left, obj[i].rect.top);
			}
			window.draw(sprite);
		}
	current_frame += 0.002 * time;
}
bool Level::СoincidenceNames(std::string name)
{
	if ((name == "chest") || (name == "key") || (name == "heart") || (name == "potion"))
		return true;
	else
		return false;
}
void Level::DrawStaticObjects(std::vector<Object> &obj, sf::RenderWindow &window, std::string name, sf::Sprite &sprite, int left, int top)
{
	for (int i = 0; i < obj.size(); i++)
	{
		if (name == "boxes")
		{
			if (СoincidenceNames(obj[i].name))
			{
				if (obj[i].find)
					sprite.setTextureRect(IntRect(left - 60, top, obj[i].rect.width, obj[i].rect.height));
				else
					sprite.setTextureRect(IntRect(left, top, obj[i].rect.width, obj[i].rect.height));
				sprite.setPosition(obj[i].rect.left, obj[i].rect.top);
				window.draw(sprite);
			}
		}
...
void Level::DrawDynamicSprite(sf::RenderWindow &window, sf::Sprite &sprite, int left, int top, int w, int h, int x, int y)
{
	sprite.setTextureRect(IntRect(left, top, w, h));
	sprite.setPosition(x, y);
	window.draw(sprite);
}